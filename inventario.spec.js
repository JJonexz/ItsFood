...
// ─────────────────────────────────────────────────────────────
//  TEST C (BONUS): Búsqueda sin resultados
// ─────────────────────────────────────────────────────────────
test('Test C (bonus) – Búsqueda sin coincidencias no muestra filas', async ({ page }) => {
  await page.goto('stock.html');

  await expect(page.locator('#stock-tbody tr').first()).toBeVisible();

  await page.locator('#search').fill('xxxxxIngredienteInexistentexxxxx');
  await page.waitForTimeout(300);

  // No debe haber filas visibles con ese texto
  const filasVisibles = page.locator('#stock-tbody tr:visible');
  const count = await filasVisibles.count();
  expect(count).toBe(0);

  console.log('✅ Búsqueda sin coincidencias: tabla correctamente vacía.');
});
